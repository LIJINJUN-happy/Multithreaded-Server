...
typedef struct Task
{
    ...
    std::list<std::string>** pWorkList;   //任务列表指针的地址（因为假如单纯复制地址则是浅拷贝）
    std::list<std::string>** pMessList;   //接受socket信息列表指针的地址
...
public:
    ...
    void AddMsgIntoTaskPool(std::list<std::string>&,int);   //把信息传进任务列表容器
